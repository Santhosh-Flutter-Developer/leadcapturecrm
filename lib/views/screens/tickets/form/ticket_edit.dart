...
class TicketEdit extends StatefulWidget {
  final String uid;
  const TicketEdit({super.key, required this.uid});
...
class _TicketEditState extends State<TicketEdit> {
  ...
  CustomerTicketModel? _ticketModel;
  ...
  Future<void> _init() async {
    try {
      _ticketModel = await TicketService.getTicket(uid: widget.uid);
      _priorityLevel = _ticketModel!.priorityLevel;
      _category = _ticketModel!.category;
      _status = _ticketModel!.status;
      _modeOfContact = _ticketModel!.modeOfContact;
      _ticketTitle.text = _ticketModel!.ticketTitle;
      _ticketDescription.text = _ticketModel!.ticketDescription;
      _clientName.text = _ticketModel!.clientName;
      _clientCompanyName.text = _ticketModel!.clientCompanyName ?? '';
      _deadline.text = _ticketModel!.deadline?.listingDateTime ?? '';
      _selectedDeadline = _ticketModel!.deadline;
      _reminder.text = _ticketModel!.reminder?.listingDateTime ?? '';
      _selectedReminder = _ticketModel!.reminder;
      _selectedAssignTo = _ticketModel!.assignTo;
      _selectedCreatedBy = _ticketModel!.createdBy;
  ...
  Widget _buildAssignmentGrid() {
    return Column(
      children: [
        UsersListDropdown(
          label: 'Assign To',
          initialValues: _initialAssignTo,
          onChangedList: (list) {
  ...
  void _submitForm() async {
    if (_formKey.currentState!.validate()) {
      try {
        ...
        await TicketService.updateTicket(uid: widget.uid, ticket: ticket);
        ...
        FlushBar.show(context, 'Ticket updated successfully', isSuccess: true);
      } catch (e, st) {
